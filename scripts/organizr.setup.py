...
path_data = {
    'data[path]': '/srv/organizr_db',
    'data[formKey]': ''
}

r_path = requests.post(url=org_url + "api/?v1/wizard_path", headers=head, data=path_data)
...
config_data = {
    ...
    'data[7][name]': 'dbName', 'data[7][value]': 'yeet',
    'data[8][name]': 'location', 'data[8][value]': '/config/www/db'
}

r_config = requests.post(url=org_url + "api/?v1/wizard_config", headers=head, data=config_data)
